...
namespace nuvelocity
{
    ...
    private:
        ...
        template<typename T>
        static auto HasInitClassInfo(int) -> decltype(T::InitClassInfo(std::declval<ClassInfo&>()), std::true_type{});
        
        template<typename T>
        static std::false_type HasInitClassInfo(...);
        
        static constexpr bool HasInitClassInfoMethod = decltype(HasInitClassInfo<Derived>(0))::value;
    ...
    protected:
        ...
        static void AddProperty(ClassInfo& info, const char* name, MemberType Derived::*memberPtr)
        {
            ...
            } else if constexpr (std::is_same_v<MemberType, int> || std::is_same_v<MemberType, int16_t>) {
                prop = new IntProperty(name, offset, size);
            } else if constexpr (std::is_same_v<MemberType, unsigned int> || std::is_same_v<MemberType, uint16_t>) {
    ...
    public:
        ...
        {
            static_assert(HasInitClassInfoMethod, 
